...
router.get('/allWaste', (req,res) => {
    const connection = getConnection();
    connection.query("SELECT k.kurum_adi, t.cinsi, a.miktar, a.id, a.bekliyor,a.ekleme_tarihi FROM atiklar a INNER JOIN atik_turleri t ON a.atik_turu=t.id INNER JOIN kullanicilar k ON k.id=a.kullaniciId Order By a.id Desc",(err,rows,fields) => {
        if(err) {
            console.log("MySql de bir hata oluştu");
            res.status(400).json({durumKodu: 400, mesaj:err.message});
        }
 ...
 * @return json
 */
router.post('/waste/recycle', (req,res) => {
    const connection = getConnection();
    const queryString = "UPDATE atiklar SET bekliyor = 0 WHERE id = ?;";
    connection.query(queryString,[
        req.body.wasteId,
    ], (err,rows,fields) => {
        if(err){
        ...
        {
            res.status(200).json({durumKodu:200, mesaj: "✔ Tebrikler"});
        }
    });
});
